...
function fetchLatLong(input, lat, long) {
    ...
    var drop = new google.maps.places.Autocomplete(input);
...
function ajaxFollow(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    //   console.log('WORKS');
    var isFollowed = _this.follow.classList.toggle('following');
    if (_this.follow.classList.contains('following')) _this.follow.textContent = 'Following';else _this.follow.textContent = 'Follow';
  }).catch(console.error);
}

exports.default = ajaxFollow;
...
function searchResultsHTML(stories) {
  return stories.map(function (story) {
    return '\n      <a href="/story/' + story.slug + '" class="search__result">\n        <strong>' + story.name + '</strong>\n      </a>\n    ';
  }).join('');
}
...
function typeAhead(search) {
  ...
  var searchResults = search.querySelector('#searchResults');
...
(0, _address2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lat'), (0, _bling.$)('#lng'));

document.querySelectorAll('.link.search').on('click', function () {
...
document.querySelectorAll('.responsive-search').on('click', function () {
    // document.querySelector('.nav').style.display = 'block';
    // document.querySelector('.nav__section--user').style.display = 'none';
    // let children = document.querySelectorAll('.nav__section--pages li:not(:last-child)');
    // for(let i=0;i<children.length;i++)
...
document.querySelectorAll('#closeSearch').on('click', function () {
...
document.querySelectorAll('.responsive-closeSearch').on('click', function () {
    // document.querySelector('.nav').style.display = 'none';
...
var followForm = (0, _bling.$$)('form.follow-user');
followForm.on('submit', _follow2.default);

document.querySelector('.side-menu').on('click', function () {
    document.querySelector('.menu').classList.add('show-me');
});

document.querySelector('.exit').on('click', function () {
    document.querySelector('.menu').classList.remove('show-me');
});

(0, _typeAhead2.default)((0, _bling.$)('#searching'));
(0, _typeAhead2.default)((0, _bling.$)('.responsive-searching'));
...
if (document.querySelector('.mychoice')) {
    document.querySelector('.mychoice').on('input', function () {
        if (document.querySelector('.mychoice').value.length > 2) document.querySelector('.mychoice').name = 'tags';else document.querySelector('.mychoice').name = '';
    });
}
